...
A busy pane is otherwise exempt from staleness, but only until its freshest turn-boundary evidence - its latest `state/<id>.turn-ended` marker or `state/<id>.busy-state` event, with the `state/<id>.meta` spawn record anchoring only a task that has none of either - reaches `FM_BUSY_TURN_MAX_SECS`; past that bound it is routed through the same wedge escalation, with the identical reason, escalation count, worktree-write deferral, and `demand-deep-inspection` marker, for inspection only - never an automatic interrupt, signal, or restart.
...
`bin/fm-crew-state.sh <id>` is the cheap current-state read for an actionable heartbeat review: it attributes an active or terminal no-mistakes run under the shared run-attribution contract, then keeps that run-step authoritative even if the pane has closed, except that the status log's own terminal report outranks a run-step label that disagrees with it.
