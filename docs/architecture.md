...
Past that count the repeats are bounded to the same `FM_PAUSE_RESURFACE_SECS` long cadence as a declared wait, because an already-deep-inspected pane re-presenting identical evidence every `FM_STALE_ESCALATE_SECS` is the alarm loop itself, while any event that resets the escalation count also restores the full escalation speed.
A stale pane whose task's status log is already effectively finished - a `done:` or `failed:` report that keyed decision verbs only closed, never contradicted - is a leftover awaiting cleanup rather than a wedge suspect, so it absorbs on that same long cadence instead of riding the escalation ladder; the log alone is never proof of done, so an authoritative working verdict or an already-armed wedge timer under it still escalates exactly as before.
...
A busy pane is otherwise exempt from staleness, but only until its freshest turn-boundary or native-progress evidence - the newest of its latest `state/<id>.turn-ended` marker, `state/<id>.busy-state` event, and `state/<id>.progress` marker, with the `state/<id>.meta` spawn record anchoring only a task that has none of them - reaches `FM_BUSY_TURN_MAX_SECS` (`bin/fm-watch.sh` owns marker selection); past that bound it is routed through the same wedge escalation, with the identical reason, escalation count, worktree-write deferral, and `demand-deep-inspection` marker for a live agent and the same dead-record report when the endpoint is proven gone, for inspection only - never an automatic interrupt, signal, or restart.
...
`bin/fm-crew-state.sh <id>` is the cheap current-state read for an actionable heartbeat review: it attributes an active or terminal no-mistakes run under the shared run-attribution contract, then keeps that run-step authoritative even if the pane has closed, except that a `blocked:` event reporting a refused or missing daemon socket outranks a potentially stale active run record only while that socket-down declaration is itself the log's latest recognized event, since any later event, including another `blocked:` one, means the crew moved on, and the status log's own terminal report outranks a run-step label that disagrees with it.
...
`bin/fm-fleet-view.sh` renders that snapshot as Markdown for humans, `bin/fm-bearings-snapshot.sh` provides the bounded bearings projection, and `bin/fm-task-dashboard.sh` projects it into the grouped local task dashboard ([dashboard.md](dashboard.md)), so the views consume one structured contract instead of reparsing raw fleet files.
...
That path merges only after one live read of the merge request confirms it is open, mergeable, conflict-free, with blocking discussions resolved and a pipeline that succeeded at the current head, and it binds the merge to that verified head; the pipeline condition alone may be waived, only where `bin/fm-pr-merge.sh` proves from GitLab's own read-only APIs that the project has no CI configuration or through its attended-only `--no-ci` flag, and the script's header owns that three-state rule; recorded metadata is never the authority for those conditions because a rebase leaves it stale.
