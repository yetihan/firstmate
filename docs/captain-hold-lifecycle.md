...
Three retained-delivery gaps remain bounded by tasks-axi 0.2.5; the first two are recorded for separate upstream work rather than representing defects introduced by this branch.
...
A retained completion link outside GitHub's `/pull/<n>` shape - a GitLab merge request, for example - cannot reach the row's structured field either, because tasks-axi records a `--pr` link only in that shape; `fm_backlog_pr_link_supported` in `bin/fm-backlog-transition-lib.sh` owns the predicate.
The link stays archived in the deliverable body line, an ordinary close or its replay adapts it into the row's note, and `answer` skips that unsupported artifact the same way it skips a relocated report.
