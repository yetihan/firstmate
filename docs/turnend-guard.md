...
When an active home instead has a live session lock held by a verified harness outside the current session's contiguous ancestry, the Claude guard emits a read-only ownership diagnostic and allows the turn to end safely.
...
That reset rides out a transiently held budget lock with the same bounded retry the auto-arm ledger write uses, because the synchronous turn-end guard publishes its block budget under this same lock at the same Stop boundary; a single-shot acquisition that lost that race used to fail the reset, which the Stop-owned auto-arm then translated into an endless exit-2 rewake loop on an otherwise healthy home (2026-09).
