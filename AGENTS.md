...
The verified harnesses are `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, and `cursor`, plus `muse` and `gemini` for crewmates and scouts only, and `nio-chat-agent` for scouts only behind its default-off channel opt-in and firstmate-side task-category gatekeeping (docs/nio-chat-agent-backend.md); never dispatch on an unverified adapter.
